...
bool Detector::init(string path, rknn_core_mask core_mask) {
...
void Detector::preprocess(Mat& data) {
...
void Detector::postprocess(vector<vector<float>>& result, float conf_thresh, float iou_thresh) {
    ...
    if (this->is_quant) {
        for (int i = 0; i < output_buffers.size(); i++) {
            ...
            int stride = sqrt(width * height / grid_h / grid_w);
            ...
            for (int gh = 0; gh < grid_h; gh++) {
                for (int gw = 0; gw < grid_w; gw++) {
                    for (int anchor_index = 0; anchor_index < anchor_num; anchor_index++) {
                        ...
                        float conf = deqntAffineToFp32(box_confidence, zp, scale);
...
bool Detector::inputImage(Mat& image) {
...
vector<vector<float>> Detector::getResult(float conf_thresh, float iou_thresh) {
