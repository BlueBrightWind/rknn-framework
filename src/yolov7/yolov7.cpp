...
bool YOLOV7::init(string path, rknn_core_mask core_mask) {
...
void YOLOV7::preprocess(Mat& data) {
...
void YOLOV7::postprocess(vector<vector<float>>& result, float conf_thresh, float iou_thresh) {
    ...
    if (this->is_quant) {
            ...
            for (int gh = 0; gh < grid_h; gh++) {
                for (int gw = 0; gw < grid_w; gw++) {
                    for (int anchor_index = 0; anchor_index < anchor_num; anchor_index++) {
                        int8_t box_confidence = input[(prop_box_length * anchor_index + 4) * grid_length + gh * grid_w + gw];
                        if (box_confidence < thres_i8)
                            continue;
                        int offset = (prop_box_length * anchor_index) * grid_length + gh * grid_w + gw;
                        int8_t* in_ptr = input + offset;
                        float box_x = deqntAffineToFp32(*in_ptr, zp, scale) * 2.0 - 0.5;
                        float box_y = deqntAffineToFp32(in_ptr[grid_length], zp, scale) * 2.0 - 0.5;
                        float box_w = deqntAffineToFp32(in_ptr[2 * grid_length], zp, scale) * 2.0;
                        float box_h = deqntAffineToFp32(in_ptr[3 * grid_length], zp, scale) * 2.0;
        ...
        for (int i = 0; i < output_buffers.size(); i++) {
            ...
            for (int gh = 0; gh < grid_h; gh++) {
                for (int gw = 0; gw < grid_w; gw++) {
                    for (int anchor_index = 0; anchor_index < anchor_num; anchor_index++) {
                        float16_t box_confidence = input[(prop_box_length * anchor_index + 4) * grid_length + gh * grid_w + gw];
...
bool YOLOV7::inputImage(Mat& image) {
...
vector<vector<float>> YOLOV7::getResult(float conf_thresh, float iou_thresh) {
