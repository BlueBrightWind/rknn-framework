...
static string model_path = "../weights/yolov5s-seg-quantify.rknn";
static string image_path = "../images/bus.jpg";
static int repeat_times = 100;

void yolov5_seg_detector_test() {
    printf("--------------------------------------------\n");
    printf("******** YOLOV5 Seg Detector Test *********\n");
    printf("--------------------------------------------\n");
...
void yolov5_seg_detector_speed() {
    printf("--------------------------------------------\n");
    printf("********* YOLOV5 Seg Speed Test ***********\n");
    printf("--------------------------------------------\n");
    YOLOV5SEG detector;
    detector.init(model_path, RKNN_NPU_CORE_0);
    Mat image = imread(image_path);
...
void yolov5_seg_async_test() {
    printf("--------------------------------------------\n");
    printf("****** YOLOV5 Seg Async Speed Test ********\n");
    printf("--------------------------------------------\n");
    function<void(AsyncYOLOV5SEG&)> putter = [&](AsyncYOLOV5SEG& detector) {
        Mat image = imread(image_path);
        int count = 0;
        while (count++ < repeat_times) {
            detector.put(image);
    ...
    function<void(AsyncYOLOV5SEG&)> getter = [&](AsyncYOLOV5SEG& detector) {
        int count = 0;
        auto t1 = system_clock::now();
        while (count++ < repeat_times) {
...
int main() {
    yolov5_seg_detector_test();
    yolov5_seg_detector_speed();
    yolov5_seg_async_test();
}